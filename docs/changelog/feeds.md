...
These feeds allow you to subscribe to updates and automatically receive new release notes as soon as 
they are published.
...
```http
GET /feed/feed.[:format]?category=buy,developer,sell
```
...
```http
GET /feed/feed.[:format]?api=developer/analytics-api,sell/analytics-api
```
